...
import crud.team.dto.comment.MyPageCommentDto;
import crud.team.dto.post.MyPagePostDto;
import crud.team.dto.post.PostSimpleDto;
...
public class MyPageService {
    ...
    public List<MyPagePostDto> findMyPost(Pageable pageable, User user) {

        Page<Post> posts = postRepository.findByUserId(pageable, user.getId());
        List<MyPagePostDto> myPagePostDtos = new ArrayList<>();
        posts.stream().forEach(i -> myPagePostDtos.add(new MyPagePostDto().toDto(i)));
        return myPagePostDtos;
    }

    // 내가 작성한 댓글 조회
    @Transactional(readOnly = true)
    public List<MyPageCommentDto> findMyComment(Pageable pageable, User user) {
        ...
        comments.stream().forEach(i -> myPageCommentDtos.add(MyPageCommentDto.toDto(i)));
        return myPageCommentDtos;
    }

    // 내가 좋아요 한 게시물 조회
    @Transactional(readOnly = true)
    public List<MyPagePostDto> findMyLike(Pageable pageable, User user) {

        Page<Like> likes = likeRepository.findByUserId(pageable, user.getId());
        List<MyPagePostDto> myPagePostDtos = new ArrayList<>();
        likes.stream().forEach(i -> myPagePostDtos.add(new MyPagePostDto().toDto(i.getPost())));
        return myPagePostDtos;
    }
}
